...
export async function getVendors() {
  const rows = await fetchAllRows(() =>
    supabase.from('exp_vendors').select('name').order('name').order('id', { ascending: true })
  )
...
export async function getCollections() {
  const rows = await fetchAllRows(() =>
    supabase.from('exp_collections').select('month_key, location, amount, id').order('id', { ascending: true })
  )
  const map = {}
  rows.forEach((r) => {
